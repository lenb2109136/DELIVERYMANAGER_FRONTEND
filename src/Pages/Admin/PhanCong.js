  ...
  return (
    <>
      ...
      <div className="mb-3 flex items-center gap-3">
        ...
        <Button disabled={data.data.length===0}
          onClick={() => {
            phanCong();
          }}
          className="ml-3"
      ...
      <div className="flex w-full flex-gap gap-2">
        <div className="w-full lg:w-4/6">
          {data.data.length===0&&<div  >
             <img src={NoData} style={{width:"160px",display:"block",margin:"0 auto"}}/>
          </div>}
          <Collapse
            defaultActiveKey={["1"]}
            ...
            items={data.data.map((v, indexs) => ({
              ...
              children: (
                <List
                  ...
                  dataSource={v.orders.map((order, idx) => ({
                    ...order,
                    _index: idx,
                  }))}
                  renderItem={(order, index) => (
                    <List.Item
                      onClick={() => setOrderClick(order)}
                      actions={[
                        <Select
                          onChange={(value) => {
                        ...
                        >
                          {data.data.map((vv, index) => (
                            <Option value={index}>
                              Cụm số {index + 1} - {vv.orders.length} đơn
                            </Option>
                          ))}
                        </Select>,
                        <a key="more">more </a>,
        ...
        <div className="w-full lg:w-2/6">
          ...
          <p className="mt-3 font-bold">
            <InboxOutlined /> Trọng lượng: &nbsp;&nbsp; {orderClick?.trongLuong}{" "}
            KG
          </p>
          <p className="mt-3 font-bold">
          ...
          <p className="mt-3 font-bold">
            <MoneyCollectFilled /> Tổng Phí: &nbsp;&nbsp; {orderClick?.fee} VND
          </p>
        </div>
      </div>
    </>
